...
| `opa-admin` (`opa-organization/opa-admin`) | Next.js 14 + shadcn/ui + Tailwind | Panel interno del equipo OPA (moderación, aprobar marcas, KPIs) — NO es para marcas ni usuarios finales | En desarrollo inicial. Entorno local preparado (2026-08-03) — ver nota abajo |
...
- ✅ `opa-mobile` y `opa-backend` — transferidos y confirmados 2026-07-03: `git remote -v` en ambos apunta a `github.com/opa-organization/...`, y funcionan (push/fetch probados).
- ✅ `opa-admin` — transferido y confirmado 2026-08-03: `git remote -v` en ese repo apunta a `github.com/opa-organization/opa-admin.git`.
...
**Nota sobre entorno local de `opa-admin` (2026-08-03):** se dejó preparado para correr — `npm install` hecho, `.env.local` creado con `NEXT_PUBLIC_SUPABASE_URL` y `NEXT_PUBLIC_SUPABASE_ANON_KEY` (mismo proyecto Supabase), verificado levantando `npm run dev` y cargando la pantalla de login sin errores. **Falta `SUPABASE_SERVICE_ROLE_KEY`** — no se puede obtener por herramienta (secreto server-side, ninguna tool de Supabase MCP la expone a propósito); hay que copiarla a mano desde el dashboard de Supabase (Project Settings → API → service_role key, proyecto `vecnktrbjolahcalkbml`) y pegarla en `opa-admin/.env.local`. Sin eso el login carga pero las queries server-side (dashboard, usuarios, marcas, moderación) van a fallar.

**Regla general de esta sección: no dar por hecho un estado "confirmado" solo porque está escrito acá — si hace falta, correr `git remote -v` de nuevo para chequear que no cambió.**
