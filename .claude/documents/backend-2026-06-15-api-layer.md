...
| `cors` | Allows requests from `localhost:3000` (opa-web dev), the opa-web production domain, and — since 2026-08-14 — **any `http://localhost:<port>` origin** via a regex match instead of a fixed port list. Was hardcoded to `localhost:8090` only (opa-mobile's pinned preview port); broke with a real CORS error the moment someone ran `npx expo start --web` without pinning a port (defaults to 8081). Rather than add a second magic port, `origin` is now a function: prod origins matched against an explicit set, anything matching `/^http:\/\/localhost:\d+$/` allowed through, everything else rejected. |
...
- [ ] **⚠️ `opa-backend` git repo is out of sync with the deployed `api` function (since 2026-08-14).** The CORS fix below (version 12) was deployed directly via the Supabase MCP's `deploy_edge_function` from an `opa-mobile`-only session that doesn't have `opa-backend` cloned — same class of drift already flagged once for a migration (see `database-2026-06-06-schema-and-seed.md`). Next session with `opa-backend` cloned: pull the deployed source (`get_edge_function` or `supabase functions download`) and commit the new `index.ts` CORS block (dynamic `localhost:*` origin match) so git matches what's actually running.
...
- [x] **CORS blocked `opa-mobile` web dev on any unpinned Expo port — fixed 2026-08-14.** The allowlist only had `localhost:8090` hardcoded (this session's preview port, pinned in `.claude/launch.json`); a user running `npx expo start --web` without that pin got Expo's real default (8081) and every call to `POST /api/brands/me/prendas` (create-garment screen) failed the CORS preflight with `ERR_FAILED`. Fixed by matching `origin` against `/^http:\/\/localhost:\d+$/` instead of one more fixed port — verified with a manual OPTIONS preflight (`Origin: http://localhost:8081`) returning `204` + `Access-Control-Allow-Origin: http://localhost:8081`. Deployed as function version 12. **See the git-drift pending item above — this is live in prod but not yet committed to `opa-backend`.**
