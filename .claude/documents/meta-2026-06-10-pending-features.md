...
- [ ] Block like/save/follow for brand accounts — per brand-system doc, a brand account "cannot like, save, or follow". Not enforced app-wide yet; the outfit scroll / product screens still expose those actions to any logged-in session including a brand. Out of scope for the brand-login task (2026-07-13).
- [ ] Multi-account switcher (personal ⇄ brand) — Instagram-style account switch; the brand account is fully independent, there is no in-app way to jump between a personal and a brand account without logging out. Not started.
